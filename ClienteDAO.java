...
public class ClienteDAO {
    private Connection conn; // Declaração da variável para armazenar a conexão com o banco de dados

    // Construtor para estabelecer a conexão com o banco de dados
    public ClienteDAO() {
        try {
            // Define a URL de conexão com o banco de dados
            String url = "jdbc:mysql://localhost:3306/seu_banco"; // substitua "seu_banco" pelo nome do seu banco
            String usuario = "seu_usuario"; // substitua "seu_usuario" pelo usuário do banco
            String senha = "sua_senha"; // substitua "sua_senha" pela senha do banco
    ...
    public void inserir(Cliente cliente) {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) { // Prepara a instrução SQL
            ...
            stmt.setDate(4, cliente.getDataCadastro()); // Atribui a data de cadastro do cliente
    ...
    public List<Cliente> listar() {
        ...
        try (Statement stmt = conn.createStatement(); // Cria um Statement para executar a consulta
            ...
            while (rs.next()) {
                ...
                cliente.setIdCliente(rs.getInt("id_cliente")); // Atribui o ID do cliente
                ...
                cliente.setDataCadastro(rs.getDate("data_cadastro")); // Atribui a data de cadastro do cliente
